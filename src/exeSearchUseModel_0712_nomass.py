...
#
#         # reference_spectra = read("../SpectraPairsData/gleams_reference_spectra.mgf", convert_arrays=1)
...
#         reference_intensity = np.array([self.bin_reference_spectrum(r.get('m/z array'), r.get('intensity array')) for r in reference_spectra])
#         # 先将500个参考谱图的点积结果计算出来
#         ndp_r_spec_list = []
#         # for x in range(50):
#         # for x in range(500):
...
#             ndp_r_spec_list.append(ndp_r_spec)
#
#         peakslist1, precursor_feature_list1, r_peakslist1 = [], [], []
#         peakslist2, precursor_feature_list2, r_peakslist2 = [], [], []
#         label_list = []
#         i, j, k = 0, 0, 0
#         for s1 in self.spectrum1:
#             s1 = self.MGF[s1]
#
#             tmp_bin_r_s1 = self.bin_reference_spectrum(s1.get('m/z array'), s1.get('intensity array'))
#             bin_r_s1 = self.caculate_nornalization_dp(reference_intensity, ndp_r_spec_list, tmp_bin_r_s1)
#             r_peakslist1.append(bin_r_s1)
...
#
#             if len(peakslist1) == 100:
#                 i += 1
#                 intensList01 = np.array(peakslist1)
#                 tmp_dplist01 = np.array(r_peakslist1)
...
#                 j = i * 100
#
#             elif (j+100) > len(self.spectrum1):
#                 num = len(self.spectrum1) - j
#                 k += 1
#                 if num == k:
#                     intensList01 = np.array(peakslist1)
#                     tmp_dplist01 = np.array(r_peakslist1)
...
#         return results
#
#     def bin_reference_spectrum(self, mz_array, intensity_array, max_mz=2500, min_mz=50.5, bin_size=0.02):
#         """
#         :param mz_array:
#         :param intensity_array:
#         :param max_mz:
...
#         return results
#
#     def caculate_nornalization_dp(self, reference, ndp_r_spec_list, bin_r_s):
#         reference_intensity = reference
#         tmp_dp_list = reference_intensity.dot(bin_r_s)
#         # 计算500个参考谱图的归一化点积
#         tmp_ndp_list = []
#         ndp_spec = np.math.sqrt(np.dot(bin_r_s, bin_r_s))
#         # for l in range(500):
#         # for l in range(50):
#         for l in range(100):
#             tmp_dp = tmp_dp_list[l] / (ndp_spec * ndp_r_spec_list[l])
#             tmp_ndp_list.append(tmp_dp)
#
#         return np.array(tmp_ndp_list)
